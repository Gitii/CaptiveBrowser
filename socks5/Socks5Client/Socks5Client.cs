...
using Socks5.Socks;
using Socks5.Socks5Client.Events;
using Socks5.TCP;
...
public class Socks5Client
{
    ...
    {
        UseAuthTypes = new List<AuthTypes>(
            new[] { AuthTypes.None, AuthTypes.Login, AuthTypes.SocksEncrypt }
        );
    }
    ...
    public bool Send(byte[] buffer, int offset, int length)
    {
        try
        {
            ...
            while (true)
            {
                byte[] outputdata = enc.ProcessOutputData(
                    buffer,
                    offst,
                    (length - offst > 4092 ? 4092 : length - offst)
                );
    ...
    public void ReceiveAsync()
    {
        if (enc.GetAuthType() != AuthTypes.Login && enc.GetAuthType() != AuthTypes.None)
        {
            Client.BeginReceive(4);
        }
        else
        {
            Client.BeginReceive(4096);
        }
    }
        ...
        try
        {
            if (enc.GetAuthType() != AuthTypes.Login && enc.GetAuthType() != AuthTypes.None)
            {
                ...
                if (recvd == torecv)
                {
                    ...
                    this.OnDataReceived(
                        this,
                        new Socks5ClientDataArgs(this, e.Buffer, e.Count, e.Offset)
                    );
                }
            }
            else
